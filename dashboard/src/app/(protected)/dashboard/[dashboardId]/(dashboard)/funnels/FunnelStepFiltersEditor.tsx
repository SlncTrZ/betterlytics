...
export function FunnelStepFiltersEditor({
  filters,
  onChange,
  globalPropertyKeys,
  ...
  const handleRemove = useCallback(
    (id: QueryFilter['id']) => {
      // Preserve "at least one filter" — replace last with an empty filter row instead of removing.
      if (filters.length <= 1) {
  ...
  return (
    <div className="py-1">
      <div
        className='grid max-h-[45vh] grid-cols-1 grid-rows-[minmax(3rem,1fr)_auto_auto] gap-2 overflow-hidden p-1 [&_[data-slot=scroll-area-scrollbar]]:!w-2'
      >
        <ScrollArea
